...
![队列的抽象数据类型](https://github.com/Aspetto33/data-structure/blob/master/images/6/%E9%98%9F%E5%88%97%E7%9A%84%E6%8A%BD%E8%B1%A1%E6%95%B0%E6%8D%AE%E7%B1%BB%E5%9E%8B.png)

### 6.10 循环队列

队列作为一种特殊的线性表，同样具有顺序存储和链式存储两种存储方式。

1）队列顺序存储的不足

会出现数组越界问题（定义两个指针，一个指针指向队头元素，一个指针指向队尾元素的下一个位置--假溢出）

![数组下标越界](https://github.com/Aspetto33/data-structure/blob/master/images/6/%E6%95%B0%E7%BB%84%E4%B8%8B%E6%A0%87%E8%B6%8A%E7%95%8C.png)

2）循环队列定义

解决假溢出的办法是后面满了，再从头开始，即头尾相接的循环。把队列的这种头尾相接的顺序存储结构称为循环队列。

计算队列长度推导：

若队列的长度最大尺寸为QueueSize，队列满的条件是（rear+1）%QueueSize == front

若rear > front，队列长度为rear - front

若rear < front，队列长度一块为QueueSize-front，另一块为rear + 0，所以队列长度为rear - front + QueueSize

因此，通用的计算队列长度公式为（rear - front + QueueSize）%QueueSize

### 6.11 队列的链式存储结构及实现

队列的链式存储结构，其实就是线性表的单链表，只不过能尾进头出，简称为链队列。

![链队列结构](https://github.com/Aspetto33/data-structure/blob/master/images/6/%E9%93%BE%E9%98%9F%E5%88%97%E7%BB%93%E6%9E%84.png)

1）队列的链式存储结构--入队操作

入队操作，其实就是在链表尾部插入结点。

![入队操作图](https://github.com/Aspetto33/data-structure/blob/master/images/6/%E5%85%A5%E9%98%9F%E6%93%8D%E4%BD%9C%E5%9B%BE.png)

![链式存储结构入队代码](https://github.com/Aspetto33/data-structure/blob/master/images/6/%E9%93%BE%E5%BC%8F%E5%AD%98%E5%82%A8%E7%BB%93%E6%9E%84%E5%85%A5%E9%98%9F%E4%BB%A3%E7%A0%81.png)

2）队列的链式存储结构--出队操作

出队操作时，就是头结点的后继结点出队，将头结点的后继改为它后面的结点，若链表除头结点外只剩一个元素时，则需将rear指向头结点。

![链式存储结构出队代码](https://github.com/Aspetto33/data-structure/blob/master/images/6/%E9%93%BE%E5%BC%8F%E5%AD%98%E5%82%A8%E7%BB%93%E6%9E%84%E5%87%BA%E9%98%9F%E4%BB%A3%E7%A0%81.png)

在可以确定队列长度最大值的情况下，建议用循环队列，无法预估队列的长度时，用链队列。
